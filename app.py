...
class PDFReport(FPDF):
    ...
    def add_table(self, df, title=None, col_width=25):
        ...
        for col in df.columns:
            self.cell(col_width, 6, str(col), 1, 0, 'C')
        self.ln()
...
def create_pdf(df_results, inputs, jobs_df):
    ...
    pdf.cell(60, 10, f"Umsatz: {kpi['Umsatz']:,.0f} EUR", 0, 0)
    pdf.cell(60, 10, f"EBITDA: {kpi['EBITDA']:,.0f} EUR", 0, 0)
    pdf.cell(60, 10, f"Cash (Y10): {kpi['Kasse']:,.0f} EUR", 0, 1)
    pdf.ln(5)
    ...
    ax2.fill_between(df_results["Jahr"], df_results["Kasse"], alpha=0.4, label="Kasse")
    ax2.plot(df_results["Jahr"], df_results["Bankdarlehen"], color="red", label="Kredit", linewidth=2)
...
st.title("Integriertes Finanzmodell: Cash Sweep")
...
with col_main_act1:
    if st.button("🔄 MODELL JETZT NEU BERECHNEN", type="primary", use_container_width=True):
        st.rerun()
with col_main_act2:
    st.info("💡 Ändern Sie Werte in den Tabs. Klicken Sie links, um zu aktualisieren.")
...
asset_types = {
    ...
    "Misc": {"price_key": None, "ul_key": "depreciation"} 
}
...
for t in range(1, 11):
    ...
    for job in valid_jobs:
        ...
        row[f"FTE {role}"] = curr_fte if curr_fte > 0 else 0.0
    ...
    asset_register["Misc"].append({
        "year": t, "amount": 1, "price": capex_misc, "total_cost": capex_misc, "ul": st.session_state["depreciation"]
    })
    capex_now += capex_misc
    
    # Job Assets
    for atype, needed in asset_needs.items():
        # Fallback für UL Keys, da diese oben im DEFAULTS Dictionary neu sind (manuell definieren wenn fehlt)
        ul_key = f"ul_{atype.lower()}" if atype != "Büro" else "ul_desk"
        price_key = f"price_{atype.lower()}" if atype != "Büro" else "price_desk"
        
        # Check defaults
        ul = st.session_state.get(ul_key, 5) # Fallback 5 Jahre
        price = st.session_state.get(price_key, 0)
        
        valid_assets = 0
        for p in asset_register[atype]:
            if (t - p["year"]) < p["ul"]:
                valid_assets += p["amount"]
        
        buy = max(0, needed - valid_assets)
        if buy > 0:
            cost = buy * price
            capex_now += cost
    ...
    for atype, purchases in asset_register.items():
        for p in purchases:
            age = t - p["year"]
            if 0 <= age < p["ul"]:
                depreciation_now += p["total_cost"] / p["ul"]
    ...
    row["EBITDA"] = ebitda
    row["EBIT"] = ebit
    row["Steuern"] = tax # WICHTIG: Hat gefehlt!
...
# Hier wird das PDF generiert, sobald das Skript durchläuft
